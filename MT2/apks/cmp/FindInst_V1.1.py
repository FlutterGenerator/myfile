        ...
        try:
            ...
            for line in instruction_srch.splitlines():
                if re.search(regex, line):
                    instruction_addr_match = re.search(r"0x[0-9A-Fa-f]+", line)
                    if instruction_addr_match:
                        instruction_addr = instruction_addr_match.group().replace("0x00", "0x", 1)
                        results.append((addr, instruction_addr))
