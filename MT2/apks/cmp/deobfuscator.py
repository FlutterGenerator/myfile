...
def RandomHelper_next(state):
    ...
    next_state = s0 + s1
    next_state = RandomHelper_rotl(next_state, 9) + s0

    s1 ^= s0
    s0 = RandomHelper_rotl(s0, 13) ^ s1
    s0 ^= np.int16(np.uint16(s1) << np.uint16(5))
...
def DeobfuscatorHelper_getString(id, chunks):
    ...
    for x in range(length):
        ...
        char_value = state >> 32 & 0xFFFF

        try:
            char = chr(char_value)
            
            # Replace escape sequences with corresponding characters
            char = re.sub(r"\\u([0-9a-fA-F]{4})", lambda m: chr(int(m.group(1), 16)), char)
            
            output.append(char)
        except UnicodeDecodeError:
            # Handle the decoding error by replacing with a suitable character
            output.append('?')
...
def DeobfuscatorHelper_getCharAt(char_index, chunks, state):
    ...
    char_value = ord(char[0])
    
    return np.int64(np.uint64(next_state) ^ (np.uint64(char_value) << np.uint16(32)))



def remove_lines(data, line_num, signature, regex):
       if args.remove_calls:
         for line_num in range(line_num, line_num - 8, -1):
                            line = data[line_num]
            ...
            for line_num, line in enumerate(data):
                ...
                if temp_output:
                    ...
                    if match:
                        ...
                        remove_lines(data, line_num, signature, regex)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--remove_calls", help="remove decryption method after restoring strings", action="store_true")
